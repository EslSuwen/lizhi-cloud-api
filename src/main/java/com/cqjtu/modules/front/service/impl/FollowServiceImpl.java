...
import com.cqjtu.modules.front.vo.FollowUser;
...
import com.cqjtu.modules.front.dao.FollowDao;
import com.cqjtu.modules.front.entity.FollowEntity;
import com.cqjtu.modules.front.service.FollowService;

import javax.annotation.Resource;

@Service("followService")
public class FollowServiceImpl extends ServiceImpl<FollowDao, FollowEntity>
  ...
  public PageUtils queryPage(Map<String, Object> params) {
    Page<FollowEntity> page =
        this.selectPage(
            new Query<FollowEntity>(params).getPage(), new EntityWrapper<FollowEntity>());
  ...
  public List<FollowUser> listFollowUser(Map<String, Object> params, Long userId) {
    ...
    List<Long> followedUserIds =
        followEntities.stream().map(e -> e.getToUserId()).collect(Collectors.toList());
    ...
    // 查询出所有的用户
    List<SysUserEntity> allUserEntitys =
        sysUserDao.selectList(
            new EntityWrapper<SysUserEntity>()
                .eq("type", UserEnum.FRONT.getType())
                .and()
                .eq("status", 1));
    Map<Long, SysUserEntity> map =
        allUserEntitys.stream().collect(Collectors.toMap(SysUserEntity::getUserId, e -> e));
  ...
  public List<FollowUser> listFollowedUser(Map<String, Object> params, Long userId) {
    ...
    Map<Long, SysUserEntity> map =
        sysUserEntities.stream().collect(Collectors.toMap(SysUserEntity::getUserId, e -> e));
    for (Long id : ids) {
      if (map.get(id) == null) {
        continue;
      }
  ...
  private List<FollowEntity> innerListFollowEntity(Map<String, Object> params, Long userId) {
    return this.selectList(
        new EntityWrapper<FollowEntity>().eq(userId != null, "from_user_id", userId));
  }
}
