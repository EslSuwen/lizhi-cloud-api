package com.cqjtu.modules.front.dao;

import com.cqjtu.modules.front.conn.HdfsConn;
...
/** User: lanxinghua Date: 2019/3/20 11:05 Desc: hdfs文件操作 */
@Repository("hdfsDao")
public class HdfsDao {
  ...
  public void copyOrMove(
      SysUserEntity user, FileEntity sourceFile, FileEntity destFile, boolean flag) {
    try {
      String sourceFormatPath = formatPathMethod(user, sourceFile);
      String destFormatPath = formatPathMethod(user, destFile);
      FileUtil.copy(
          HdfsConn.getFileSystem(),
          new Path(sourceFormatPath),
          HdfsConn.getFileSystem(),
          new Path(destFormatPath),
          flag,
          true,
